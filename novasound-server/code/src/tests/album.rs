...
mod tests {
    ...
    #[tokio::test]
    async fn test_get_album_by_id_rejects_unknown_database_album_type() {
        let pool = get_test_pool!();
        let artist_id = "artist-invalid-type";
        let album_id = "album-invalid-type";
        insert_artist(&pool, artist_id, "Test Artist").await;
        insert_album(
            &pool,
            album_id,
        ...
        let result = album_service::get_album_by_id(&pool, album_id).await;

        assert!(matches!(
            result,
            Err(novasound_application::errors::AppError::Database(_))
        ));
    }
